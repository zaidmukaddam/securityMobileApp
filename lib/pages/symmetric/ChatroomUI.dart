...
class _ChatRoomUIState extends State<ChatRoomUI> {
    ...
    return new WillPopScope(
      ...
      child: new Scaffold(
          appBar: AppBar(
            ...
            title: Text(
                "ChatRoom: ${Provider.of<MessagingService>(context).channel}"),
          ),
          body: SafeArea(
            child: Container(
    ...
    return Stack(
      children: [
        Positioned(
            ...
            bottom: 80,
            top: 0,
            child: Container(child: _getMessageList())),
        Positioned(
          bottom: 0,
          right: 0,
          left: 0,
            ...
            child: Row(
              children: [
                Container(
                    ...
                    child: buildCustomInputField(true, "write something",
                        _message, 60, false, false, this)),
                IconButton(
                    icon: Icon(
                      Icons.send,
                      color: Colors.red,
  ...
  void _sendMessage() {
    Provider.of<MessagingService>(context, listen: false).sendMessage(
        SymmetricUtil.encrypt("", _message.text.toString(), widget.password),
        widget.room);
    _message.text = "";
  }
  ...
  Widget itemBuilder(BuildContext context, int index) {
    Message message = Provider.of<MessagingService>(context, listen: false)
        .messages
        .reversed
        .elementAt(index);
    Widget mess;
    if (message.senderID == userName)
      mess = _getListItemMe(message);
    else
      mess = _getListItemOther(message);
    return GestureDetector(
      ...
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => MessageDetailSymUI(
                  message: message, password: widget.password)),
        );
      },
    );
  }
