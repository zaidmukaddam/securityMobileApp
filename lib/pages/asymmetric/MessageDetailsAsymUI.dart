...
class MessageDetailAsymUI extends StatefulWidget {
  ...
  const MessageDetailAsymUI({Key key, this.message, this.publicKey})
      : super(key: key);
  @override
...
class _MessageDetailAsymUIState extends State<MessageDetailAsymUI> {
      ...
      child: Container(
        child: Column(
          ...
          children: [
            ...
            Text(
              "TimeStamp : ${widget.message.date.microsecondsSinceEpoch}",
              style: buttonStyle,
            ),
            Text(
              "Encryption Algorithm : RSA 2048 - SHA256 PSS",
              style: buttonStyle,
            ),
            SizedBox(
              height: 50,
            ...
            buildCustomInputField(false, "Your Message", _message, inputHeight,
                true, false, this),
            SizedBox(
              height: 50,
            ),
            ...
            buildCustomInputField(false, "Your Message", _signature,
                inputHeight, true, false, this),
            SizedBox(
              height: 50,
            ),
            Text(
            ...
            isValidating
                ? getContainer(CircularProgressIndicator(), inputHeight / 3)
                : buildCustomInputField(false, "Your Message",
                    _validationResult, inputHeight / 3, false, false, this),
          ],
        ),
      ),
    );
  ...
  void validateSignature() async {
    ...
    try {
      isValid =
          await AsymmetricUtil.validate(message.trim(), sig, widget.publicKey);
    } catch (e) {}
    isValidating = false;
