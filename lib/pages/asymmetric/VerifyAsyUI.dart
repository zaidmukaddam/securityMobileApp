...
class _VerifyAsyUIState extends State<VerifyAsyUI> {
  ...
  _getBody(BuildContext context) {
    double inputHeight = MediaQuery.of(context).size.height / 4.2;
    return Column(
      children: [
        buildCustomInputField(
            true, "Message", _input, inputHeight, false, true, this),
        buildCustomInputField(
            true, "Signature", _signature, inputHeight, false, true, this),
        buildCustomInputField(
            true, "Public Key", _password, inputHeight / 2, true, false, this,
            callback: (v) => {setState(() {})}),
        ButtonBuilder.buildRaisedButton(context, _verify, "Verify signature",
            active: _password.text.isNotEmpty),
        buildCustomInputField(
            true, "", _output, inputHeight / 3, false, false, this)
      ],
    );
  }
  ...
  _verify() async {
    ...
    var isValide = await RSA.verifyPSS(_signature.text, _input.text,
        Hash.HASH_SHA256, SaltLength.SALTLENGTH_AUTO, public);
