...
class CustomEncoder {
  ...
  static String encodeToBinary(String s) {
    List<String> binary =
        s.codeUnits.map((int strInt) => strInt.toRadixString(2)).toList();
    return binary.join(" ");
  }
  ...
  static String decodeFromBinary(String s) {
    ...
    for (String i in binary)
      if (i.length != 7 && i.length != 6) return "Invalid format";
    if (binary.isEmpty) return "";
    List<int> result =
        binary.map((String strInt) => int.parse(strInt, radix: 2)).toList();
  ...
  static String encodeToAscii(String s) {
    List<String> binary =
        s.codeUnits.map((int strInt) => strInt.toString()).toList();
    return binary.join(" ");
  }
