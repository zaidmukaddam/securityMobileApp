  ...
  void initChannel(String channel) {
    ...
    if (channel != null) {
      socketIO = SocketIOManager()
          .createSocketIO(socketServer, '/', query: 'chatID=$channel');
      socketIO.init();
      socketIO.subscribe('receive_message', (jsonData) async {
        ...
        Message message = Message(
            ...
            data['senderChatID'],
            data['receiverChatID'],
            new DateTime.fromMicrosecondsSinceEpoch(data['sentAt']));
        if (!isAsync) {
          ;
          messages.add(ExtendedMessage(message.text, message.senderID,
              message.receiverID, message.date, message.text, "", ""));
        } else {
          for (User user in users) {
            if (user.username == message.senderID) {
              Message m = await AsymmetricUtil.decrypt(
                  message, user.publicKey, myPrivateKey);
              messages.add(m);
              break;
            }
          }
    ...
    if (isAsync) {
      for (User user in users) {
        if (user.username == receiverChatID) {
          text = await AsymmetricUtil.encryptAndSign(
              text, user.publicKey, myPrivateKey);
          break;
        }
      }
    }
    messages.add(ExtendedMessage(
        text, username, receiverChatID, DateTime.now(), originalText, "", ""));
    discussions = getDiscussions();
