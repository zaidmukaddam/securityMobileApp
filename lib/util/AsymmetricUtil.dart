...
class AsymmetricUtil {
  static Future<String> encrypt(String message, String publicKey) async {
    var result =
        await RSA.encryptOAEP(message, "", Hash.HASH_SHA256, publicKey);
    return result;
  }

  static Future<String> sign(String message, String privateKey) async {
    var result = await RSA.signPSS(
        message, Hash.HASH_SHA256, SaltLength.SALTLENGTH_AUTO, privateKey);
    return result;
  }
  ...
  static Future<ExtendedMessage> decrypt(
      Message message, String publicKey, String privateKey) async {
    ...
    for (int i = 0; i < text.length / blockSize; i++) {
      empty = empty +
          await RSA.decryptOAEP(
              text.substring(i, (i + 1) * blockSize).padRight(blockSize),
              "",
              Hash.HASH_SHA256,
              privateKey);
    }
    return ExtendedMessage(message.text, message.senderID, message.receiverID,
        message.date, empty.trim(), privateKey, publicKey);
  }

  static Future<bool> validate(
      String message, String signature, String publicKey) async {
    var isValide = await RSA.verifyPSS(signature, message, Hash.HASH_SHA256,
        SaltLength.SALTLENGTH_AUTO, publicKey);
    return isValide;
  }
}
